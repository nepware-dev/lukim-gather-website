...
import {
  getProjectNameFromFormData,
  GET_SURVEY_DATA,
} from "@containers/CustomForms";
...
const getFullName = (createdBy: any) =>
...
const Dashboard = () => {
  ...
  const [selectInputCategory, setSelectInputCategory] =
    useState<SelectInputType | null>(null);
  const [selectInputRegion, setSelectInputRegion] =
    useState<SelectInputType | null>(null);
  const [selectInputProject, setSelectInputProject] =
    useState<SelectInputType | null>(null);
  const [selectInputProtectedArea, setSelectInputProtectedArea] =
    useState<SelectInputType | null>(null);
  ...
  const currentDate = formatISO(new Date(), { format: "basic" }).replace(
    /\+|:/g,
    ""
  );
      ...
      data.happeningSurveys?.filter(
        (item: {
          ...
          if (
            ...
            item.protectedArea?.id !== selectInputProtectedArea.id
          ) {
            return false;
          }
          if (
            selectInputProject &&
            item.project?.id !== selectInputProject.id
          ) {
            return false;
          }
  ...
  const customFormGeoJSON: any = useMemo(() => {
    ...
    if (customSurveyData?.survey?.length > 0) {
      shape = customSurveyData?.survey.reduce(
        (features: any, survey: FormDataType) => {
          if (
            status === "My Entries" &&
            survey?.createdBy &&
            survey?.createdBy?.id !== userId
          ) {
            return features;
          }
          ...
          if (selectInputRegion) {
            const provinceData: any = findPropertyAnywhere(
              formAnswers,
              "province"
            );
            if (
              provinceData?.replace(/_/g, " ")?.toLowerCase() !==
              selectInputRegion.title.toLowerCase()
            ) {
              return features;
            }
          }
          const locationGPS: string =
            findPropertyAnywhere(formAnswers, "location_gps") || "";
          if (locationGPS) {
  ...
  const flatCustomForms = useMemo(
    () =>
      customFormGeoJSON.features.map(
        ({
          properties: {
            customForm: { formAnswers: formData },
          },
        }: any) => flattenObject(formData)
      ),
    [customFormGeoJSON]
  );
  ...
  const onClick = useCallback((event) => {
    const cluster = event.features.find(
      (feat: mapboxgl.EventData) =>
        feat.layer.id === "clusters" || feat.layer.id === "clusters-form"
    );
    if (cluster) {
      ...
      const source: mapboxgl.GeoJSONSource = mapRef?.current?.getSource(
        cluster.source
      ...
      const pointFeatures = event.features.filter(
        (feat: mapboxgl.MapboxGeoJSONFeature) =>
          feat.layer?.id === "unclustered-point" ||
          feat.layer?.id === "unclustered-form-point"
      );
      if (pointFeatures.length === 0) {
        ...
        pointFeatures.forEach((pFeat: mapboxgl.MapboxGeoJSONFeature) => {
          ...
          if (pFeat.properties?.customForm) {
            ...
            if (
              !featureItems.some(
                (ftItem) =>
                  ftItem.formAnswers && ftItem.id === customFormData.id
              )
            ) {
        ...
        const newPopupData = (
          <div
            className={`max-h-[22rem] overflow-y-auto ${
              featureItems.length === 1 ? "py-2px" : "pt-[12px]"
            }`}
          >
            {featureItems.map((item: any) => {
              if (item?.formAnswers?.section_1) {
                const popUpData = Object.entries(
                  item.formAnswers.section_1
                )?.map(([key, val]) => {
                  if (val === "") {
                    return null;
                  }
                ...
                return (
                  <div
                    key={item.id}
                    className="border border-b-0 last:border-b only:border-0 border-color-border p-[6px] only:p-[2px]"
                  >
                    <h6 className="text-[14px] font-medium mb-[4]">
                      {item.title}
                    </h6>
                    {popUpData}
  ...
  const onLoad = useCallback(() => {
    surveyCategory.filter((cat) =>
      cat.childs.filter((categoryIcon) =>
        mapRef?.current?.loadImage(categoryIcon.icon, (error, res: any) => {
    ...
    surveyExportData.map((surveyItem: SurveyDataType) => {
      ...
      return {
        ...
        attachment: surveyItem.attachment
          .map((mediaUrl) => JSON.stringify(mediaUrl))
          .join(","),
  ...
  const regionOptions = useMemo(
    () =>
      regions?.regions.map(({ name: title, ...item }: { name: string }) => ({
        title,
        ...item,
      })),
    [regions]
  ...
  const projectOptions = data?.happeningSurveys
    ...
    .filter(
      (item: SurveyDataType["project"], index: boolean, self: any) =>
      ...
      <div className={classes.mapWrapper} ref={contentRef}>
        ...
        >
          <Source
            type="vector"
            tiles={[
              "https://cms.lukimgather.org/protected_area_tiles/{z}/{x}/{y}",
            ]}
          >
            <Layer {...protectedAreaLayerTitle} />
            <Layer {...protectedAreaLayer} />
          </Source>
          <Source id="surveyPolySource" type="geojson" data={surveyPolyGeoJSON}>
            <Layer {...polygonTitle} />
            <Layer {...polygon} />
          </Source>
          <Source
          ...
          >
            ...
            <Layer
              {...unclusteredPointLayer}
              beforeId={clusterCustomFormLayer.id}
            />
          </Source>
          {popup && (
            <Popup
